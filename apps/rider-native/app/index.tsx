...
import { type NearbyDeliveryJob } from '@/data/nearbyJobsRepository';
import { emptyIncomingJobQueue, hydrateIncomingOffers } from '@/services/incomingOfferHydration';
import { type IncomingJobQueueState } from '@/domain/incomingJobQueue';
...
import {
  addRiderOfferNotificationListeners,
  ensurePushReadiness,
  getLastRiderOfferNotificationSubId,
...
export default function RiderHomeScreen() {
  ...
  const [incomingQueue, setIncomingQueue] = useState<IncomingJobQueueState<NearbyDeliveryJob>>(emptyIncomingJobQueue);
  const [incomingMessage, setIncomingMessage] = useState<string | null>(null);
  const [error, setError] = useState<string | null>(null);
  const openedOfferId = useRef<string | null>(null);
  ...
  const openIncomingJobs = useCallback((subId?: string | null) => {
    const params = subId ? { subId } : undefined;
    router.push(params ? { pathname: '/nearby-jobs', params } : '/nearby-jobs');
  }, [router]);

  const refreshIncomingOffers = useCallback(async (
    activeSession?: RiderSession | null,
    options: { focusSubId?: string | null; autoOpen?: boolean } = {},
  ) => {
    const usableSession = activeSession ?? session;
    if (!usableSession || !riderFeatures.deliveryV3Accept) return null;

    try {
      const result = await hydrateIncomingOffers(incomingQueue, {
        session: usableSession,
        focusSubId: options.focusSubId,
      });
      setIncomingQueue(result.queue);
      setIncomingMessage(result.message);
      const activeOffer = result.queue.active;
      if (options.autoOpen && activeOffer && openedOfferId.current !== activeOffer.sub_id) {
        openedOfferId.current = activeOffer.sub_id;
        openIncomingJobs(activeOffer.sub_id);
      }
      if (!activeOffer) openedOfferId.current = null;
      return result;
    } catch (cause) {
  ...
  }, [incomingQueue, openIncomingJobs, session]);
  ...
  async function restoreAccount() {
    ...
    try {
      ...
      if (profile?.is_online) {
        await syncOnlineDeviceState(activeSession, profile);
        const notificationSubId = await getLastRiderOfferNotificationSubId().catch(() => null);
        await refreshIncomingOffers(activeSession, { focusSubId: notificationSubId, autoOpen: true });
      }
    } catch (cause) {
      setAccountState('blocked');
  ...
  useFocusEffect(
    useCallback(() => {
      if (!session || !rider?.is_online) return undefined;
      void refreshIncomingOffers(session, { autoOpen: true });
      return undefined;
    }, [refreshIncomingOffers, rider?.is_online, session]),
  );
  ...
  }, [refreshIncomingOffers, session, rider?.id, rider?.is_online]);

  useEffect(() => {
    if (!session || !rider?.is_online) return;
    return addRiderOfferNotificationListeners({
      onReceived: (subId) => {
        void refreshIncomingOffers(session, { focusSubId: subId, autoOpen: true });
      },
      onResponse: (subId) => {
        void refreshIncomingOffers(session, { focusSubId: subId, autoOpen: true });
      },
    });
  }, [refreshIncomingOffers, session, rider?.is_online]);
  ...
  async function signInWithLine() {
    ...
    try {
      ...
      if (profile?.is_online) {
        await syncOnlineDeviceState(newSession, profile);
        await refreshIncomingOffers(newSession, { autoOpen: true });
      }
    } catch (cause) {
      setSession(null);
      setRider(null);
  ...
  async function toggleOnline() {
    ...
    try {
      ...
      if (profile?.is_online) await refreshIncomingOffers(session, { autoOpen: true });
    } catch (cause) {
      setError(cause instanceof Error ? cause.message : String(cause));
    } finally {
  ...
  async function logout() {
    ...
    try {
      ...
      setIncomingQueue(emptyIncomingJobQueue);
    ...
    <SafeAreaView style={styles.safeArea}>
      <ScrollView style={styles.scrollView} contentContainerStyle={styles.container} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.primaryActions}>
          <Pressable accessibilityRole="button" disabled={!isOnline || !riderFeatures.deliveryV3Accept} onPress={() => openIncomingJobs(incomingQueue.active?.sub_id)} style={[styles.incomingButton, (!isOnline || !riderFeatures.deliveryV3Accept) && styles.buttonDisabled]}>
            <Text style={styles.incomingButtonTitle}>งานเข้า</Text>
            <Text style={styles.incomingButtonText}>{incomingQueue.active ? `${incomingQueue.active.shop_name}${incomingQueue.queued.length ? ` · คิว ${incomingQueue.queued.length}` : ''}` : 'เปิดดูและรับงานใหม่'}</Text>
          </Pressable>
        ...
        {incomingQueue.active && (
          <Pressable accessibilityRole="button" onPress={() => openIncomingJobs(incomingQueue.active?.sub_id)} style={styles.incomingStateCard}>
            <Text style={styles.incomingStateLabel}>มีงานเข้าใหม่</Text>
            <Text style={styles.incomingStateTitle}>{incomingQueue.active.shop_name}</Text>
            <Text style={styles.incomingStateMeta}>
              แตะเพื่อเปิดงานเต็มจอ{incomingQueue.queued.length ? ` · มีอีก ${incomingQueue.queued.length} งานในคิว` : ''}
            </Text>
          </Pressable>
        )}

        {!incomingQueue.active && incomingMessage && isOnline && (
          <Text style={styles.incomingMessage}>{incomingMessage}</Text>
        )}
...
const styles = StyleSheet.create({
  ...
  incomingStateCard: { gap: 5, padding: 16, borderRadius: 8, backgroundColor: '#153F2B', borderWidth: 1, borderColor: '#70D69B' },
  incomingStateLabel: { fontSize: 12, fontWeight: '900', color: '#8EE6B0' },
  incomingStateTitle: { fontSize: 21, fontWeight: '900', color: '#FFFFFF' },
  incomingStateMeta: { fontSize: 13, lineHeight: 19, color: '#D6E8DD' },
  incomingMessage: { fontSize: 13, lineHeight: 19, color: '#667085' },
