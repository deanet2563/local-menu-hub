...
class MyTreeLineLoginModule : Module() {
  ...
  override fun definition() = ModuleDefinition {
    ...
    AsyncFunction("login") { channelId: String, promise: Promise ->
      ...
      pendingNonce = nonce
      activity.startActivityForResult(LineLoginApi.getLoginIntent(activity, channelId, params), REQUEST_CODE_LINE_LOGIN)
    }

    OnActivityResult { _, payload ->
      if (payload.requestCode != REQUEST_CODE_LINE_LOGIN) return@OnActivityResult
      ...
      when (result.responseCode) {
        ...
        LineApiResponseCode.CANCEL -> promise.reject("E_LINE_CANCELLED", "LINE Login was cancelled", null)
        else -> {
