...
export const cart = {
  add(item: AddCartItem, opts?: { force?: boolean; allowMultipleShops?: boolean }): "ok" | "different_shop" {
    ...
    const forceSwitching = switchingShop && !!opts?.force;
    let items = forceSwitching ? [] : state.items;
    ...
    // Force-switch (cart cleared to a new single shop) always adopts the new
    // shop. The allowMultipleShops path (cart not cleared) preserves whatever
    // primary shopId is already set, only defaulting to the new item's shop
    // when the cart was previously empty.
    const shopId = forceSwitching ? item.shopId : (state.shopId ?? item.shopId);
    set({ shopId, items });
    return "ok";
  },
