...
export function ShopPage({ shopId }: { shopId: string }) {
  ...
  return (
    <div className="pb-28">
      ...
      {cartCount(c) > 0 && <Link to="/cart" search={(prev) => prev} className="customer-floating-above-nav fixed z-30 left-4 right-4 rounded-xl bg-orange-500 text-white px-4 py-3 flex justify-between text-sm font-medium"><span>ดูตะกร้า ({cartCount(c)})</span><span>฿{cartTotal(c)}</span></Link>}
