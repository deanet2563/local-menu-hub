...
// into each set. A set is only grouping; price is always the sum of products.
// ============================================================
...
export function ShopPage({ shopId }: { shopId: string }) {
  ...
  const qtyOf = (id: string) => c.items
    .filter((i) => i.itemId === id && i.setId === activeSetId)
    .reduce((n, i) => n + i.qty, 0);
  const activeSetCount = c.items
    .filter((i) => i.setId === activeSetId)
    .reduce((n, i) => n + i.qty, 0);
    ...
    <div className="pb-28">
      ...
      <div className="sticky top-0 z-20 bg-white border-b border-orange-100 px-4 py-3">
        ...
        <p className="mt-1 text-[11px] text-gray-400">
          {activeSetCount > 0 ? `${activeSetName} มี ${activeSetCount} ชิ้น · เลือกสินค้าเพิ่มได้อิสระ` : `${activeSetName} · เลือกสินค้าอะไรก็ได้จากเมนูด้านล่าง`}
        </p>
      </div>
