-- MyTree Ordering Flow v2 — transactional wrapper around the proven fn_create_order.
-- Additive only: the existing fn_create_order is not modified.
-- The Worker validates all prices/options/bundles before calling this RPC.

create or replace function public.fn_create_order_v2(
  p_shop_id text,
  p_items jsonb,
